...
TestFile::TestFile()
{
    ...
    log_level = realm::util::Logger::Level::TEST_ENABLE_CORE_LOGGING_LEVEL;
    logger_factory = [](realm::util::Logger::Level level) {
        return std::make_shared<util::StderrLogger>(level);
    };
    ...
    , m_should_teardown_test_directory(config.should_teardown_test_directory)
{
    ...
    app_config.log_level = config.app_log_level;
